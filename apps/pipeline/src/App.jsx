...
// ════════════════════════════════════════════════
// DATE RANGE PICKER (Triple Whale style)
// ════════════════════════════════════════════════
      ...
      {tab === "overview" && (
        <div className="animate-fade">
          ...
          {(() => {
            ...
            const VARIABLE_OPTS = ["Hook", "Lead", "Unique Mechanism", "Body/VSL", "CTA/Close", "Offer", "Landing Page", "Avatar/Angle", "Format"];
            const HOOK_TYPE_OPTS = ["Short Gundry", "Pattern Interrupt", "Testimonial Lead", "UGC Native", "Curiosity/Question", "Bold Claim", "News/Authority", "Story-based", "Controversy", "Before/After"];
            const VSL_BODY_OPTS = ["Problem-Agitate-Solve", "Demo/Proof", "Testimonial Stack", "Educational", "Comparison", "Before/After", "Story Arc", "Listicle"];
            const usedUMPs = [...new Set(allAds.map(a => a.strategy?.ump).filter(Boolean))];
            const usedUMSs = [...new Set(allAds.map(a => a.strategy?.ums).filter(Boolean))];
            const usedOffers = [...new Set(allAds.map(a => a.strategy?.offer_name).filter(Boolean))];
            const usedLPs = [...new Set(allAds.map(a => a.strategy?.landing_page_name).filter(Boolean))];
            const rowS = { display: "flex", alignItems: "center", padding: "8px 0", borderBottom: "1px solid var(--border-light)" };
            const labelS = { width: 140, flexShrink: 0, fontSize: 12, color: "var(--text-muted)", fontWeight: 450 };
      ...
      {tab === "metrics" && (
        <div className="animate-fade">
          <div style={{ marginBottom: 14 }}>
            <DateRangePicker dateFrom={mDateFrom} dateTo={mDateTo} onChange={(from, to) => { setMDateFrom(from); setMDateTo(to); }} />
          </div>
  ...
  return (
    <div className="app-layout">
      ...
      <div className="main-content">
        ...
        {page === "analytics" && <AnalyticsPage ads={ads} th={th} />}
